...
import { LayoutDashboard, Users, Dog, GraduationCap, Landmark, CalendarDays, Sun, Moon } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen w-72 border-r flex flex-col p-8 z-50 transition-colors ${
          ...
          <button
            ...
            className={`w-full flex items-center space-x-4 px-6 py-4 rounded-2xl font-bold uppercase text-[10px] tracking-widest transition-all ${
              activeTab === item.id 
                ...
                : (darkMode ? 'text-slate-500 hover:bg-slate-900 hover:text-slate-300' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600')
            }`}
          >
            {item.icon}
      ...
      <button 
        ...
        className={`mt-auto flex items-center justify-between p-4 rounded-2xl border transition-all ${
          darkMode ? 'bg-slate-900 border-slate-800 text-emerald-500' : 'bg-slate-50 border-slate-200 text-slate-400'
        }`}
      >
