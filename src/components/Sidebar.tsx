...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    ...
    { id: 'secretariat', label: 'Secrétariat', icon: <FileText size={20} /> }, // Ajouté
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen w-72 border-r flex flex-col p-8 z-50 transition-colors ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-100'}`}>
      <div className="flex items-center gap-3 mb-12 px-2">
        ...
        <h1 className={`text-sm font-black italic leading-none tracking-tighter uppercase ${darkMode ? 'text-white' : 'text-slate-800'}`}>Amicale Canine Vernoise</h1>
      </div>
      <nav className="flex-1 space-y-2">
        {menuItems.map((item) => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center space-x-4 px-6 py-4 rounded-2xl font-bold uppercase text-[10px] tracking-widest transition-all ${activeTab === item.id ? 'bg-emerald-500 text-white shadow-lg' : (darkMode ? 'text-slate-500 hover:bg-slate-900' : 'text-slate-400 hover:bg-slate-50')}`}>
            {item.icon}
