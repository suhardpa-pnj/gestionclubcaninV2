...
import { LayoutDashboard, Users, Dog, GraduationCap, Landmark, FileText, CalendarDays, Sun, Moon } from 'lucide-react';
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen w-72 border-r flex flex-col p-8 z-50 transition-colors ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-100'}`}>
      <div className="flex items-center gap-3 mb-10 px-2">
        ...
        <h1 className={`text-[11px] font-black italic leading-none tracking-tighter uppercase ${darkMode ? 'text-white' : 'text-slate-800'}`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center space-x-4 px-5 py-3.5 rounded-2xl font-bold uppercase text-[9px] tracking-widest transition-all ${
              activeTab === item.id 
      ...
      <button onClick={toggleDarkMode} className={`mt-6 flex items-center justify-between p-4 rounded-2xl border transition-all ${darkMode ? 'bg-slate-900 border-slate-800 text-emerald-500' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
