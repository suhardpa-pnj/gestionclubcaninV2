...
interface ClubState {
  ...
  updateDogPhoto: (dogId: string, photoUrl: stringimport { create } from 'zustand';
import { db } from '../firebase/config';
...
export const useStore = create<ClubState>((set, get) => ({
  ...
  seedBoutique: async () => {
    ...
    // Nettoyage radical des anciens produits pour supprimer les doublons
    ...
    refs.forEach(p => {
      const pRef = doc(db, "products", p.id); // ID FIXE ICI
      newBatch.set(pRef, { ...p, stock: 10, weight: "20kg" });
    });

    await newBatch.commit();
    alert("✅ Boutique nettoyée et réinitialisée proprement !");
    get().fetchData();
  },
