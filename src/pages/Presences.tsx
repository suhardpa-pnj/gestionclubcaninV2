...
import { 
  ...
  Image as ImageIcon, Trash2, Globe 
} from 'lucide-react';

const Presences = () => {
  const { members, dogs, attendances, darkMode, addAttendance, deleteAttendance } = useStore();
  ...
  // CONFIGURATION DES TERRAINS AVEC CODES COULEURS DU PLAN
  const terrainsACV = [
    { id: 'Principal - Zone A', label: 'Principal A', color: '#06b6d4' }, // Cyan
    { id: 'Principal - Zone B', label: 'Principal B', color: '#f97316' }, // Orange
    { id: 'Chiots', label: 'Chiots', color: '#3b82f6' },              // Bleu
    { id: 'Agility', label: 'Agility', color: '#eab308' },            // Jaune
    { id: 'Du Fond', label: 'Terrain du Fond', color: '#ef4444' },    // Rouge
        ...
        {attendances.map((att) => {
          ...
          return (
            <div key={att.id} className={`p-8 rounded-[40px] border transition-all relative group ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-emerald-50 shadow-xl'}`}>
              ...
              <h3 className={`text-xl font-serif italic mb-4 ${darkMode ? 'text-white' : 'text-[#1B4332]'}`}>{(att.presentDogIds?.length || 0)} Chiens présents</h3>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-slate-400 text-[10px] font-bold uppercase tracking-tight"><User size={14} className="text-[#BC6C25]" /> {coach?.firstName || 'Moniteur'}</div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                  <button type="button" onClick={() => setShowMap(!showMap)} className="text-[#BC6C25] hover:underline text-[9px] font-black uppercase flex items-center gap-2">
                    <ImageIcon size={14} /> {showMap ? 'Masquer le plan' : 'Voir le plan du club'}
                  </button>
                ...
                {showMap && (
                  <div className="rounded-[2rem] overflow-hidden border-4 border-white shadow-2xl animate-in zoom-in-95 duration-300">
                    <img src="terrain.jpg" alt="Plan du club" className="w-full h-auto object-cover" />
                  </div>
                )}
